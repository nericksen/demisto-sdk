...
class PytestLinter(DockerBaseLinter):
    ...
    def process_docker_results(self, container_obj, container_exit_code, test_image: str) -> LinterResult:
        ...
        Returns:
            TODO
        """
        linter_result: LinterResult = super().process_docker_results(container_obj, container_exit_code, test_image)
        ...
        if container_exit_code != self.FAILED_UNIT_TESTS_ERROR_CODE:
            self.__class__.SUCCESSFUL_UNIT_TESTS[self.package.name()][test_image] = test_reports
        else:
            self.__class__.FAILURE_UNIT_TESTS[self.package.name()][test_image] = test_reports
    ...
    @classmethod
    def report_unit_tests(cls, verbose: bool):
        if verbose and cls.FAILURE_UNIT_TESTS:
            click.secho(f'\n###########\nUnit Tests\n###########', fg='yellow')
        cls._report_successful_packages(verbose)
        cls._report_unsuccessful_packages(verbose)
