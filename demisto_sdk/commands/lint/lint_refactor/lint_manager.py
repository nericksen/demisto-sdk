...
class LintManager:
    ...
    # def _get_packages_from_pack_object(self, pack_object: GeneralObject) -> List[Union[Integration, Script]]:
    #     if pack_object.type() ==

    def _get_packages(self, content_repo: git.Repo, input_dirs: str, git: bool, all_packs: bool, base_branch: str) \
            -> List[Path]:
        ...
        """
        # if input_dirs:
        #     input_objects: List = [path_to_pack_object(file_path) for file_path in input_dirs.split(',')]
        #     pkgs = []
        ...
        packages_to_test: list
        if all_packs or git:
            content: Content = Content(content_repo.working_dir)
            integrations: List[Integration] = [integration for pack in content.packs.values() for integration in
                                               pack.integrations]
            scripts: List[Script] = [script for pack in content.packs.values() for script in pack.scripts]
            all_packages: List[Union[Script, Integration]] = integrations + scripts
    ...
    @staticmethod
    def _get_all_packages(content_dir: Path) -> List[Union[Script, Integration]]:
        """Gets all integration, script in packages and packs in given content repo.

        Returns:
            list: A list of integration, script and beta_integration names.
        """
        content: Content = Content(content_dir)
        integrations: List[Integration] = [integration for pack in content.packs.values() for integration in
                                           pack.integrations]
        scripts: List[Script] = [script for pack in content.packs.values() for script in pack.scripts]
        all_packages: List[Union[Script, Integration]] = integrations + scripts
    ...
    @staticmethod
    def get_packages(all_packs: bool = False, dir_path: Optional[str] = None):
        if all_packs
